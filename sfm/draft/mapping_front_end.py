        ...
        # for i in range(2):
            ...
            for k in self.img_pose[i].kp_matches:
                if self.img_pose[i].kp_match_exist(k,i+1):
                    ...
                    kp_src_idx_.append(k)

            #  Check matched feature points
            # print("src",src.shape)
            ...
            dst_norm = dst
            src_norm = src    
            E, mask = cv2.findEssentialMat(dst_norm,src_norm,cameraMatrix = self.calibration,method =cv2.RANSAC,prob=0.999,threshold=1)


            new_mask = []
            for j,t in enumerate(mask):
                if t == 1:
                    new_mask.append(j)
            src_norm = src_norm[new_mask]
            dst_norm = dst_norm[new_mask] 

            # print(src_norm.shape)
            # Check if E is correct
            for j,src in enumerate(src_norm):

                src_ = np.hstack((src,[1]))
                dst_ = np.hstack((dst_norm[j],[1]))
                # print("_src:",_src,"_dst:",_dst)
                zero = np.dot(np.dot(dst_,E),src_.T)
                print("zero =", zero)


            #  Check matched feature points after essential matrix
            # print("feature points:",len(mask))

            _, local_R, local_t, mask=cv2.recoverPose(E,dst_norm,src_norm)
            # local_R, local_t = cv2.recoverPose(E,dst_norm,src_norm, cameraMatrix=self.calibration,mask = mask)
            new_mask = []
            kp_src_idx = []
            for j,t in enumerate(mask):
                if t == 255:
                    new_mask.append(j)
                    kp_src_idx.append(kp_src_idx_[j])
            src_norm = src_norm[new_mask]
            dst_norm = dst_norm[new_mask] 


            # Check E=t^R*s
            # t_x = np.array([[1,-local_t[2],local_t[1]],
            # [local_t[2],0,-local_t[0]],
            # [-local_t[1],local_t[0],0]])
            # _E=np.dot(t_x,local_R)
            # print("E'",_E*E[0][0]/_E[0][0])
            # print("E",E,"\n")


            #  Check matched feature points after recover pose
            # print("feature points:",len(mask))
            ...
            points4D = cv2.triangulatePoints(projMatr1=self.img_pose[i].P,projMatr2=self.img_pose[i+1].P,projPoints1=src_norm.T, projPoints2=dst_norm.T)
            points4D = points4D / np.tile(points4D[-1, :], (4, 1))
            pt3d = points4D[:3, :].T

            # fig = plt.figure()
            # ax = fig.add_subplot(111, projection='3d')
            # ax.scatter(pt3d[:,0], pt3d[:,1], pt3d[:,2], c='skyblue', s=60)
            # ax.set_xlabel('x')
            # ax.set_ylabel('y')
            # ax.set_zlabel('z')
            # plt.legend()
            ...
            for landmark in self.landmark:
                if(landmark.seen>=3):
                    landmark.point/=(landmark.seen - 1)
...
if __name__ == "__main__":
    ...
    # kp_data = fe.read_kp_cvs_file('datasets/4.5/mapping/kp/0_1_keypoints.csv')
    # desc_data = fe.read_desc_cvs_file('datasets/4.5/mapping/desc/0_1_descriptors.csv')
