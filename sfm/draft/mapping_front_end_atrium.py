...
class MappingFrontEnd(object):
    ...
    def superpoint_generator(self, image):
        ...
        superpoints_, descriptors_, heatmap = self.fe.run(image)
        ...
        #     print(heatmap[point[1].astype(int)][point[0].astype(int)])
    ...
    def feature_matching(self,save = False):
        ...
        #     for j in range(1,2):
                ...
                F_, inliers= cv2.findFundamentalMat(src_points,dst_points,cv2.FM_RANSAC,3, 0.9)
                print(len(inliers))
                good_match_count = 0
                for k,good_match in enumerate(inliers):
                    if good_match  > -1:  #or matches[2,k]>0.50:
                ...
                kp_data =  np.hstack((src_points,dst_points,matches.T,inliers))
    ...
    def initial_estimation(self):
        ...
        # for i in range(2):
            ...
            src_norm = np.expand_dims(src, axis=1)
            dst_norm = np.expand_dims(dst, axis=1) 



            # print(src_norm[:,0,:])
            # print(dst_norm[:,0,:])           
            # E, mask = cv2.findEssentialMat(dst_norm,src_norm,cameraMatrix = self.calibration,method =cv2.FM_RANSAC,prob=0.999,threshold=0.005)
            E, mask = cv2.findEssentialMat(dst_norm,src_norm,cameraMatrix = self.calibration,method =cv2.LMEDS,prob=0.9999)
            ...
            dst_norm = np.expand_dims(new_dst, axis=1) 
            src_norm = np.expand_dims(new_src, axis=1) 
            kp_src_idx = new_kp_src_idx   
           
            # print(mask)
            print(E)
            _, local_R, local_t, mask=cv2.recoverPose(E,dst_norm,src_norm,cameraMatrix = self.calibration)
            ...
            local_R = np.identity(3,dtype=np.float)
            local_t = np.array([[1.38],[0],[-0.29]],dtype=np.float)
            ...
            self.img_pose[i+1].P = np.dot(self.calibration,np.hstack((cur_R.T,-np.dot(cur_R.T,cur_t))))
            ...
            s = np.dot(self.img_pose[i].P,points4D)
            s/=s[2]
            d=np.dot(self.img_pose[i+1].P,points4D)
            d/=d[2]
            # print(self.img_pose[i+1].T)
            # print(self.img_pose[i+1].P)
            # Find good triangulated points
            for j,k in enumerate(kp_src_idx):
                if(mask[j]):
                    match_idx = self.img_pose[i].kp_match_idx(k, i+1)
                    # if pt3d[j][2]>20 or pt3d[j][2]<0:
                    #    print(np.around(pt3d[j],decimals=2),points4D.T[j])
                    #    print(src_norm[j,0,:],s.T[j])
                    #    print(dst_norm[j,0,:],d.T[j])
    ...
    def bundle_adjustment(self):
        ...
        for i,idx in enumerate(img_pose_id_list):   
            ...
            if i < 2:
                ...
                graph.add(gtsam.PriorFactorPose3(X(idx), gtsam.Pose3(
                wRc, gtsam.Point3(1.38*idx, 0, -0.29*idx)), posePriorNoise)) 
                # i+=1
