...
class MappingFrontEnd(object):
    ...
    def initial_estimation(self):
        for i in range(len(self.img_pose)-1):
            ...
            self.img_pose[i+1].P = np.dot(self.calibration,np.hstack((cur_R.T,-1*np.dot(cur_R.T,cur_t))))

            points4D = cv2.triangulatePoints(projMatr1=self.img_pose[i].P,projMatr2=self.img_pose[i+1].P,projPoints1=src, projPoints2=dst)
...
#    formatter={'float_kind':'{:0.2f}'.format})
    ...
    # undistort_image('datasets/4.5/mapping/', '*.jpg')
