...
from texture_feature_matching.configs.myconfig_2x3x8 import *
...
def run():
    ...
    if run_feature_extraction:
        feature_matcher.feature_extraction(undistort_img_size, feature_type, nn_thresh)
    # Feature Matching
    if run_feature_matching:
        feature_matcher.feature_matching(
            undistort_img_size, feature_type, matching_type, calibration_matrix, threshold, 0.6)
    if run_generate_g2o:
        rotation_dict = read_rotation_data(basedir,number_images)
    ...
    if run_bundle_adjustment:
        ...
        back_end = MappingBackEnd(basedir, number_images, calibration_matrix,
                                  pose_estimates, measurement_noise, pose_prior_noise, filter_bad_landmarks_enable, min_obersvation_number, prob, threshold, backprojection_depth,prior_indices)
        # Bundle Adjustment
        tic_ba = time.time()
        ...
        # Plot Result
        plot_with_result(sfm_result, 15, 15, 15, 0.5)
