...
def run():
    ...
    wRc = np.array([[1, 0, 0], [0, 0, 1], [0, -1, 0]])
    delta = np.radians(45)
    # Create pose estimates
    pose_estimates = [Pose3(Rot3(np.dot(wRc,
                                        # cRc'
                                        np.array([[math.cos(delta*i), 0, -math.sin(delta*i)],
                                                  [0, 1, 0],
    ...
    data_directory = 'feature_matcher/library_data/bright/'
    num_images = 8
